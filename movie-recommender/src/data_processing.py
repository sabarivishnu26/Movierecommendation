...
def load_ratings(path=None):
    """
    Load raw ratings, auto-detecting the MovieLens format:
      - ml-latest-small style: ratings.csv, comma-separated, header row present,
        columns userId,movieId,rating,timestamp
      - ml-1m style: ratings.dat, "::" separated, NO header row,
        columns UserID::MovieID::Rating::Timestamp
    Both are normalized to the same userId/movieId/rating/timestamp schema so
    every downstream function works unchanged regardless of which dataset was used.
    """
    if path is None:
        csv_path = os.path.join(RAW_DATA_DIR, "ratings.csv")
        dat_path = os.path.join(RAW_DATA_DIR, "ratings.dat")
        if os.path.exists(csv_path):
            ...
            raise FileNotFoundError(
                f"No ratings file found at {csv_path} or {dat_path}. "
                "See data/raw/README.md for setup instructions."
            )

    if path.endswith(".dat"):
        # ml-1m format: UserID::MovieID::Rating::Timestamp, no header
        ratings = pd.read_csv(
            path,
            sep="::",
            engine="python",
