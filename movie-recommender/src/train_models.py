...
def main():
    ...
    movies_df = None
    movies_csv = os.path.join("data", "raw", "movies.csv")
    movies_dat = os.path.join("data", "raw", "movies.dat")
    if os.path.exists(movies_csv):
        movies_df = pd.read_csv(movies_csv)
    elif os.path.exists(movies_dat):
        # ml-1m format: MovieID::Title::Genres, no header, latin-1 encoded
